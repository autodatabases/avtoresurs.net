...
from tecdoc.views.parts import PartTypeGroupSupplierList
...
urlpatterns = [
    ...
    url(r'^(?P<mnf_id>\d+)/(?P<model_id>\d+)/(?P<type_id>\d+)/(?P<section_id>\d+)/$', PartTypeGroupSupplierList.as_view(), name='part_list'),
    #     name='parts-catalog'),
]
